...
RESTful ["POST"] ["tracker"] = ( request, response ) =>
{
	...
	if ( TRACKERS_ID [request.headers.auth] && TRACKERS_ID [request.headers.auth] == tracker_id ) {
		// It will accept post requests with the maximun body size of 1KB (1000 B)
		util. getRequestBody( request, 1000, ( body ) => { 
			try {
...
{
	log.info( request.method + " request to " + request.url );
